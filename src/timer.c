...
void timebase_pwm(void)
{
	...
	TIM_OCInitStructure.TIM_Pulse = period/2-1;
	TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_Low;
	TIM_OCInitStructure.TIM_OCNPolarity = TIM_OCPolarity_Low;
	TIM_OCInitStructure.TIM_OCIdleState = TIM_OCIdleState_Set;
	TIM_OCInitStructure.TIM_OCNIdleState = TIM_OCIdleState_Set;
	TIM_OC1Init(XCLK_TIM, (TIM_OCInitTypeDef *)&TIM_OCInitStructure);
	TIM_OC1PreloadConfig(XCLK_TIM, TIM_OCPreload_Enable);
...
void timebase_toggle(void)
{
	...
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;
	TIM_TimeBaseStructure.TIM_Period = ((APB2CLK / XCLKFREQ ) - 1);
	TIM_TimeBaseStructure.TIM_ClockDivision = 0;
	TIM_TimeBaseStructure.TIM_RepetitionCounter = 0;
	TIM_TimeBaseInit(XCLK_TIM, (TIM_TimeBaseInitTypeDef *)&TIM_TimeBaseStructure);
	TIM_PrescalerConfig(XCLK_TIM, 0, TIM_PSCReloadMode_Immediate);
	...
	TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Disable;		// povolit spravny vystup
	TIM_OCInitStructure.TIM_OutputNState = TIM_OutputState_Enable;		// povolit spravny vystup
	TIM_OCInitStructure.TIM_Pulse = ((APB2CLK / XCLKFREQ ) - 1);
	TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_Low;			// nastavit polaritu
	TIM_OCInitStructure.TIM_OCNPolarity = TIM_OCPolarity_High;			// nastavit polaritu
	TIM_OCInitStructure.TIM_OCIdleState = TIM_OCIdleState_Set;			// nastavit idle state
	TIM_OCInitStructure.TIM_OCNIdleState = TIM_OCIdleState_Set;			// nastavit idle state
	TIM_OC1Init(XCLK_TIM, (TIM_OCInitTypeDef *)&TIM_OCInitStructure);
	TIM_OC1PreloadConfig(XCLK_TIM, TIM_OCPreload_Enable);
